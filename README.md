...
2. Pull the Docker image by running the following command:
   ...
   ```bash
   docker pull przemo145235/radiology-report-triage
   ```

3. After the image is downloaded, run the application using:

   ```bash
   docker run -p 8000:8000 przemo145235/radiology-report-triage
   ```

This will start the application, and it will be accessible on `http://localhost:8000`.

## 💻 System Requirements
- **Operating System:** Windows 10/11, macOS Mojave or later, or recent Linux distribution.
- **Processor:** Minimum of Dual-Core.
- **Memory:** At least 4 GB of RAM.
- **Storage:** At least 100 MB of free disk space.
- **Network:** Internet connection for downloading dependencies and models.

## 📊 Features
- **Zero-Shot Classification:** Classifies reports without any prior training.
- **UMAP Visualization:** Offers visual representation of data clustering and relationships.
- **Docker Support:** Quick setup with Docker, allowing easy deployment.

## 🛠️ Usage Instructions
Once downloaded and installed, you can begin using the application by following these steps:

1. Open your web browser and go to `http://localhost:8000`.
2. Upload your radiology report in a compatible format (e.g., .txt).
3. Click on the "Classify" button. 
4. Review the results, which will indicate whether the report is classified as Normal or Abnormal.

## 👩‍⚕️ Use Case Scenarios
This application fits well for:
- Radiologists needing quick report assessments.
- Medical professionals who handle large volumes of clinical texts.
